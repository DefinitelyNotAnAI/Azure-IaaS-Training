...
(function () {
  ...
  const STORAGE_KEY    = 'iaas-workshop-participant';
  const TRACKED_MODULES = ['welcome', 'module1', 'module2', 'module3', 'wrapup'];
  const STATUS_LABELS  = {
    not_started:   'Not started',
    started:       'Started',
    complete:      'Complete',
  ...
  function defaultStatuses() {
    const s = {};
    TRACKED_MODULES.forEach(function (m) { s[m] = 'not_started'; });
    return s;
  }
  ...
  function loadParticipant() {
    try {
      ...
      if (!raw) return null;
      const p = JSON.parse(raw);
      // Invalidate if the session has been rotated
      if (!p.sessionId || p.sessionId !== window.APP_CONFIG.sessionId) return null;
      return p;
    } catch (_) {
      return null;
  ...
  // Redirect to check-in if no valid participant; returns participant or null.
  function requireParticipant() {
    const p = loadParticipant();
    if (!p) { window.location.href = 'index.html'; return null; }
    return p;
  ...
  async function onCheckin(email, displayName) {
    ...
    if (existing && existing.email === normalizedEmail) {
      // Returning participant in same session — keep progress, refresh display name
      participant = Object.assign({}, existing, {
    ...
    // TODO M2 — POST /api/participants and merge server response:
    // try {
    //   const res = await fetch('/api/participants', {
    //     method: 'POST',
    //     headers: {
    //       'Content-Type': 'application/json',
    //       'x-session-code': window.APP_CONFIG.sessionCode,
    //     },
    //     body: JSON.stringify({ email: normalizedEmail, displayName: participant.displayName }),
    //   });
    //   if (res.ok) {
    //     const data = await res.json();
    //     participant = Object.assign(participant, data, { sessionId: window.APP_CONFIG.sessionId });
    //   }
    // } catch (err) { console.warn('[workshop] API unavailable, using local state', err); }
  ...
  function updateModuleStatus(moduleKey, status) {
    ...
    // TODO M2 — PATCH /api/participants/{email}:
    // fetch('/api/participants/' + encodeURIComponent(p.email), {
    //   method: 'PATCH',
    //   headers: {
    //     'Content-Type': 'application/json',
  ...
  function saveFeedback(feedback) {
    ...
    // fetch('/api/participants/' + encodeURIComponent(p.email), {
    //   method: 'PATCH',
    //   headers: { 'Content-Type': 'application/json', 'x-session-code': window.APP_CONFIG.sessionCode },
    //   body: JSON.stringify({ feedback: p.feedback }),
    // }).catch(function (err) { console.warn('[workshop] feedback patch failed', err); });
  }
  ...
  function renderProgressBar() {
    ...
    modules.forEach(function (key) {
      const cfg    = window.APP_CONFIG.modules[key];
      const status = p ? (p.moduleStatuses[key] || 'not_started') : 'not_started';
      const seg    = document.createElement('div');
      seg.className = 'progress-segment status-bg-' + status;
      // Show short label: "Networking" from "Module 1 — Networking"
      seg.textContent = (cfg.label.split('—')[1] || cfg.label).trim();
      seg.title       = cfg.label + ': ' + STATUS_LABELS[status];
      bar.appendChild(seg);
    });
  }
  ...
  function renderWrapupSummary() {
    ...
    const items = [
      ...
      { key: 'module1', label: 'Module 1 — Networking' },
      { key: 'module2', label: 'Module 2 — Compute' },
      { key: 'module3', label: 'Module 3 — Storage' },
      { key: 'wrapup',  label: 'Wrap-up' },
    ];
  ...
  document.addEventListener('DOMContentLoaded', function () {
    const snEl = document.getElementById('session-name');
    if (snEl) snEl.textContent = window.APP_CONFIG.sessionName;
