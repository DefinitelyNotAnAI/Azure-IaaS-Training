...
(function () {
  ...
  const MODULE_LABELS = { module1: 'M1 Networking', module2: 'M2 Compute', module3: 'M3 Storage' };
  const STATUS_LABELS = {
    not_started:   'Not started',
    started:       'Started',
    complete:      'Complete',
  ...
  async function fetchParticipants(code) {
    // TODO M2 — replace stub with real API call:
    // const res = await fetch('/api/admin/participants', {
    //   headers: { 'x-access-code': code },
    // });
    // if (res.status === 401) return null;        // bad code
    // if (!res.ok) throw new Error('API ' + res.status);
    // return res.json();

    // M1 stub: return empty array so the table and regroup cards render correctly
  ...
  function renderRegroupSummary(participants) {
    ...
    MODULES.forEach(function (key) {
      ...
      card.className = 'regroup-card';
      card.innerHTML =
        '<h3>' + MODULE_LABELS[key] + '</h3>' +
        '<div class="regroup-counts">' +
          '<div class="regroup-count-block">' +
            '<div class="regroup-count count-complete">' + counts.complete + '</div>' +
            '<div class="count-label">Complete</div>' +
          '</div>' +
          '<div class="regroup-count-block">' +
            '<div class="regroup-count count-started">' + counts.started + '</div>' +
            '<div class="count-label">Started</div>' +
          '</div>' +
          '<div class="regroup-count-block">' +
            '<div class="regroup-count count-need_help">' + counts.need_help + '</div>' +
            '<div class="count-label">Need Help</div>' +
          '</div>' +
        '</div>';
      container.appendChild(card);
    });
  ...
  function renderParticipantTable(participants, filterStatus) {
    ...
    if (filtered.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" style="text-align:center;color:#605e5c;padding:1.5rem">' +
        (participants.length === 0 ? 'No participants yet. Data appears here once the API is connected.' : 'No participants match the filter.') +
        '</td></tr>';
      return;
    }
  ...
  async function loadDashboard(code, filterStatus) {
    const statusEl = document.getElementById('refresh-status');
    if (statusEl) statusEl.textContent = 'Refreshing…';

    try {
      const participants = await fetchParticipants(code);
      if (participants === null) {
        showAccessGate('Invalid access code. Please try again.');
        return;
      }
  ...
  function startAutoRefresh(code) {
    ...
    refreshTimer = setInterval(function () {
      const sel = document.getElementById('filter-status');
      loadDashboard(code, sel ? sel.value : null);
    }, 30000);
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if (form) {
      form.addEventListener('submit', async function (e) {
        ...
        currentCode = code;
        showDashboard();
        await loadDashboard(code, null);
        const autoEl = document.getElementById('auto-refresh-toggle');
        if (!autoEl || autoEl.checked) startAutoRefresh(code);
      });
    }
    ...
    if (refreshBtn) {
      refreshBtn.addEventListener('click', function () {
        if (!currentCode) return;
        const sel = document.getElementById('filter-status');
        loadDashboard(currentCode, sel ? sel.value : null);
      });
    }
    ...
    if (filterSel) {
      filterSel.addEventListener('change', function () {
        if (currentCode) loadDashboard(currentCode, filterSel.value || null);
      });
    }
    ...
    if (autoToggle) {
      autoToggle.addEventListener('change', function () {
        if (autoToggle.checked && currentCode) startAutoRefresh(currentCode);
        else stopAutoRefresh();
      });
    }
  });
