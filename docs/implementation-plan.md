...
> **Module numbering follows the instructor guide** (see the per-module instructor scripts in this `docs/` folder), which is the live run-of-show. The three hands-on labs are **Module 1 = Networking, Module 2 = Compute, Module 3 = Storage**. **Platform & Architecture is an instructor-led demo**, not a numbered participant lab. This differs from the older [website-plan.md](website-plan.md) (which listed Platform as Module 1 and ran four modules) — see Section 11 to reconcile that document.
...
```json
{
  "PartitionKey": "contoso-2026-01-01",
...
- [x] Architecture diagram / reference link for the Platform & Architecture framing page: the official **Azure landing zone conceptual architecture** ([CAF — What is an Azure landing zone?](https://learn.microsoft.com/en-us/azure/cloud-adoption-framework/ready/landing-zone/)), captured in [sandbox-modules.md](sandbox-modules.md). Swap for a custom/customer-branded slide later if desired.
